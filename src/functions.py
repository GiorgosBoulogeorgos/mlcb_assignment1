...
Original file is located at
    https://colab.research.google.com/drive/1kFBb_mgp-7RluZhr9MKTeYuSHPBh8sIS
"""
...
def split_data(df, target_col='age', test_size=0.2, seed=42):
    ...
    Uses binning to allow stratification on the continuous 'age' target.
    """
...
def calculate_metrics(y_true, y_pred):
    """Calculates the four required regression metrics."""
...
def bootstrap_eval(y_true, y_pred, n_resamples=1000, seed=42):
    """
    Performs bootstrap resampling on validation predictions.
...
def stability_selection(X, y, feature_names, n_subsamples=50, subsample_fraction=0.8, top_n=200, threshold=0.5, seed=42):
    """
    Implements stability selection by perturbing the training data.
    """
    np.random.seed(seed)
    n_samples = X.shape[0]
...
def evaluate_proxy_model(X_train, y_train, X_val, y_val, feature_mask):
    """
    Evaluates a feature subset using ElasticNet (at defaults) as a proxy model.
    """
...
def optuna_tune_model(model_name, pipeline, X_train, y_train, n_trials=40, cv=5, seed=42):
    """
    Tunes models using Optuna (Tree-structured Parzen Estimator).
...
def calculate_class_metrics(y_true, y_pred, y_prob):
    """Calculates the required classification metrics."""
...
def bootstrap_eval_class(y_true, y_pred, y_prob, n_resamples=1000, seed=42):
    """
    Performs bootstrap resampling on validation/evaluation predictions for classification.
    """
    np.random.seed(seed)
    n_samples = len(y_true)
    bootstrap_results = []
